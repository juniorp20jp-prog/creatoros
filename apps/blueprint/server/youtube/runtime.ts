...
import { SystemClock, UuidGenerator, YouTubeAuthorizationService } from "../../core";
import { createAnalysisRunPrismaClient, PrismaSessionRepository, PrismaUserRepository, PrismaYouTubeAuthorizationRepository, requireDatabaseUrl } from "../../core/persistence/prisma";
...
type YouTubeRuntime = Readonly<{ handlers: YouTubeHttpHandlers; disconnect(): Promise<void> }>;
...
export function getYouTubeAuthorizationRuntime(): YouTubeRuntime {
  ...
  const compositionFactory = (): Promise<YouTubeCompositionResult> => {
    compositionPromise ??= createComposition(repository, clock, ids);
    return compositionPromise;
  };
  activeRuntime = { handlers: new YouTubeHttpHandlers({ currentSession, compositionFactory, authorizationStates: createProcessYouTubeAuthorizationStateStore(clock), clock, ids, production: process.env.NODE_ENV === "production" }), disconnect: owned.disconnect };
  return activeRuntime;
}

async function createComposition(repository: PrismaYouTubeAuthorizationRepository, clock: SystemClock, ids: UuidGenerator): Promise<YouTubeCompositionResult> {
  ...
  try {
    ...
    return { status: "success", value: { protocol, service: new YouTubeAuthorizationService(repository, protector, protocol, clock, ids) } };

...
type CreatorIntelligenceWorkspaceProps = {
  ...
  locale: Locale;
  scenario: {
    id: CreatorIntelligenceScenarioId;
    channelName: string;
...
function isSuccessfulViewModel(
  viewModel: CreatorIntelligenceViewModel,
): viewModel is CreatorIntelligenceSuccessViewModel {
  return (
...
export function CreatorIntelligenceWorkspace({
  content,
  locale,
  scenario,
  scenarios,
  viewModel,
}: CreatorIntelligenceWorkspaceProps) {
  ...
  const activeScenario = scenarios.find(
    (option) => option.id === scenario?.id,
  );

  return (
    <div className={styles.workspace} lang={locale}>
      <IntelligenceHeader
        channelName={scenario?.channelName ?? content.common.notAvailable}
        content={content}
        locale={locale}
      ...
      {successfulViewModel ? (
        <div aria-live="polite" className={styles.intelligenceContent}>
          <ExecutiveBrief
            brief={successfulViewModel.brief}
            content={content}
          />

          {!successfulViewModel.hasInsights ? (
            <IntelligenceNoInsightsState content={content} />
          ) : null}

          <InsightGrid
            content={content}
            eyebrow={content.insights.priorityEyebrow}
            insights={successfulViewModel.priorityInsights}
            locale={locale}
            title={content.insights.priorityTitle}
            titleId="priority-insights-title"
          />
          <InsightGrid
            content={content}
            eyebrow={content.insights.additionalEyebrow}
            insights={successfulViewModel.additionalInsights}
            locale={locale}
            title={content.insights.additionalTitle}
            titleId="additional-insights-title"
          />
          <EvidenceSection
            content={content}
            evidence={successfulViewModel.evidence}
            locale={locale}
          />
          <AnalysisContext
            content={content}
            context={successfulViewModel.context}
            locale={locale}
            scenarioLabel={activeScenario?.label ?? content.common.notAvailable}
          />
          ...
          <AnalysisLimitations
            content={content}
            limitations={successfulViewModel.limitations}
          />

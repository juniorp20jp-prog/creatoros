...
export type MetricCollectionOutcome = "completed" | "no-change" | "partial";
export type MetricObservationSource = "youtube-channel-sync" | "youtube-video-sync";
export type MetricObservationProvenance = "provider-sync" | "baseline-from-current-state";
export type MetricAvailability = "available" | "partial" | "authorization-revoked" | "unavailable";
export type MetricObservationBatch = Readonly<{ batchId: string; userId: string; channelId: string; sourceSyncId: string; sourceType: MetricObservationSource; provider: "youtube"; observedAt: string; collectionOutcome: MetricCollectionOutcome; availability: MetricAvailability; coverageCount: number; coverageLimit?: number; truncated: boolean; schemaVersion: typeof HISTORICAL_METRICS_SCHEMA_VERSION; provenance: MetricObservationProvenance; createdAt: string }>;

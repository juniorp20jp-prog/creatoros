...
import { AUTH_TEST_EXPIRY, AUTH_TEST_TIME, AuthenticationTestClock, identityInput, userInput } from "./fixtures/authentication-fixtures";
...
test("AuthenticationService creates a session for an active internal identity", async () => {
  ...
  const result = await harness.authentication.authenticateIdentity({ identityId: identityInput.identityId, sessionId: "auth_test_authenticated", expiresAt: AUTH_TEST_EXPIRY, metadata: { clientType: "web", locale: "es" } });
...
test("AuthenticationService returns a safe failure for a missing identity", async () => {
  const result = await services().authentication.authenticateIdentity({ identityId: "missing", sessionId: "auth_test_missing", expiresAt: AUTH_TEST_EXPIRY, metadata: { clientType: "web" } });
...
test("AuthenticationService rejects disabled identities and inactive users", async () => {
  ...
  const first = await disabledIdentity.authentication.authenticateIdentity({ identityId: identityInput.identityId, sessionId: "auth_test_disabled", expiresAt: AUTH_TEST_EXPIRY, metadata: { clientType: "web" } });
  ...
  const second = await inactiveUser.authentication.authenticateIdentity({ identityId: identityInput.identityId, sessionId: "auth_test_inactive", expiresAt: AUTH_TEST_EXPIRY, metadata: { clientType: "web" } });
